...
### Overview: This script creates a set of data structures that are able to hold
...
# data set. It is important to note that, although the survey data function can read in 
...
### same basic function as in create_surveyIDs_fun.R
...
###    a. you do this by clicking "Addins" > "Paste as Tribble"

test<-as.data.frame(c("https://docs.google.com/spreadsheets/d/1aZftMqxQLyLdmF9r4cIwppJbwkkYeoJsYUeoIcMKGy8/edit#gid=167419926", "https://docs.google.com/spreadsheets/d/1s-zb_3u3q6UScv__M8UVB2RaNjcD5wypKSscmGZO5w8/edit#gid=167419926", "https://docs.google.com/spreadsheets/d/1UXfsN_n7QO8z0vhiYwnNrfI1_uh57aFQyMKOmd1uK8U/edit#gid=167419926", "https://docs.google.com/spreadsheets/d/1uvLHTrR2TmUjjvTMC0_5kBP4zTkf7TJHxw-ien1hWGQ/edit#gid=167419926", "https://docs.google.com/spreadsheets/d/1KtGSScDTeniOyx2vu8ZxQgX9niAhAB-Hx1Duig-vFbE/edit#gid=167419926", "https://docs.google.com/spreadsheets/d/1yR8VnwdGF-pBFlT6zxJwbhpXl116RaYC5eIhLQ11cQY/edit#gid=167419926", "https://docs.google.com/spreadsheets/d/1_0xJPqwSrJF087-5CgE6oTiVgNVjmmeZXeHhow1oPec/edit#gid=167419926"))
names(test)[1]<-"link"

surveylookup_wNew <- tibble::tribble(
  ~school_fromFileName, ~link, ~cdeSchoolNumber, ~respondents,    ~type, ~endYear, ~final,
                "CCMS",    NA,            1262L,     "raters", "rating",    2023L,     1L,
                 "CES",    NA,            7950L,     "raters", "rating",    2023L,     1L,
            "Harrison",    NA,            3802L,     "raters", "rating",    2023L,     1L,
             "Lincoln",    NA,            5166L,     "raters", "rating",    2023L,     1L,
            "McKinley",    NA,            5704L,     "raters", "rating",    2023L,     1L,
          "Washington",    NA,            9248L,     "raters", "rating",    2023L,     1L,
                "CCHS",    NA,            1266L,     "raters", "rating",    2023L,     1L
  )%>% 
  mutate(cdeSchoolNumber = as.character(cdeSchoolNumber)) %>% 
  select(-link)%>%
  bind_cols(test)%>%
  ...
  bind_rows(surveylookup) %>% 
  create_surveyID()
saveRDS(surveylookup_wNew,"surveylookup_wNew.rds")
...
write.csv(optionsOldwithNew, "data/optionsQlookup.csv")
#### !!!!!!!!  MANUAL step when questions have a new set of options !!!!!!!! ##
#### manually set optionOrder of options in spreadsheet if they are new
#### Opening up the "data/optionsQlookup.csv" sheet in google or excel is proably the easiest.
...
if(checked == TRUE){saveRDS(optionsQlookup, "data/optionsQlookup.rds")}
checked <- TRUE
...
getDataFromAllSheets <- function(dataPath = surveylookup$link[1],
  ...
  # dataPath = "https://docs.google.com/spreadsheets/d/1UXfsN_n7QO8z0vhiYwnNrfI1_uh57aFQyMKOmd1uK8U/edit#gid=167419926"
  # endyear = 2023
  # dimensions = ratingDimensions
  # final = 1
  # schoolNumber = "3802"
