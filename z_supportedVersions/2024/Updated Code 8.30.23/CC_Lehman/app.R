...
ui <- tagList( 
  ...
  navbarPage( 
    ...
    ## end title  
    header = tagList(absolutePanel(top = "3em", left = "1em" , tags$style(type='text/css', ".selectize-input { padding: 2px; min-height: 0; font-size: 12px;} .selectize-dropdown { line-height: 9px; font-size: 12px; padding: 3px;}"),
                                   # uiOutput("masterSelect") 
                                   ),
                     div(
...
# Rating overall plot -----------------------------------------------------
  ...
  output$overallRatingPlot <- renderggiraph({
    colorScale <- input$surveyColor_btn
    mList <- process_ratingData(level = "overall", schoolNum = "0000", selectedconstruct=NA)
    print(mList$m)
  ...
  ratingData <- reactive({
    ...
    process_ratingData(level = "construct", schoolNum = school1, selectedconstruct=NA)
...
# Rating plot - schools ---------------------------------------------------
  ...
  output$ratingPlot <- renderggiraph({
    colorScale <- input$surveyColor_btn
    m <- ratingData()$m %>% 
...
# Rating plot - subconstructs ---------------------------------------------
  ...
  subConstructData <- reactive({
    ...
    process_ratingData(level = "subconstruct", schoolNum = school1, selectedconstruct=construct)
    
  })
  
  output$subconstructPlot <- renderggiraph({
    colorScale <- input$surveyColor_btn
...
output$ratinglong <- renderGirafe({
  ...
  validate(need(school1 != "0000", "Please select a school."))
  dataList <- process_ratingData(level = "construct", schoolNum = school1, selectedconstruct=NA) # this produces a list
...
#### district plots -------------
  ...
  output$surveyDistrict <- renderggiraph({
   colorScale <- input$surveyColor_btn
    xAxis <- 'endYear'
